...
OTHER_PROPERTIES = {
    'sasl_username': _("username to authenticate as."),
    'sasl_password': _("password to use for SASL authentication"),
...
def _get_libvirt_connection(driver_info):
    ...
    try:
        ...
        raise exception.LibvirtError(err=e)
...
def _parse_driver_info(node):
    ...
    if sasl_username and sasl_password and ssh_key_filename:
        raise exception.InvalidParameterValue(_(
            "LibvirtPower requires one and only one of authentication, "
            "(sasl_username, sasl_password) and ssh_key_filename to be set.")
            % ssh_key_filename)
...
def _set_boot_device(conn, domain, device):
    ...
    :param domain: libvirt domain object.
    :raises: LibvirtErr if failed update domain xml.
    """
...
class LibvirtPower(base.PowerInterface):
    ...
    def get_power_state(self, task):
        ...
        :raises: NodeNotFound if could not find a VM corresponding to any
            of the provided MACs.
        :raises: LibvirtErr if failed to connect to the Libvirt uri.
        """
